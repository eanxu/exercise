...
func (dtile *DataSetTile)CreateTile(ds gdal.Dataset, dsdatatype gdal.DataType, bands []int, tileSize int, r, w geoQuery.GQ,
	querysize int, resampling string, bandmin, bandmax, nodatavalues []float64)  {
	var hasEmptyCell bool
	memDrv, err := gdal.GetDriverByName("MEM")
	fileDrv, err := gdal.GetDriverByName("GTiff")
	if err != nil {
		log.Fatal("GetDriverByName MEM error, ", err)
	}
	alphaband := ds.RasterBand(1).GetMaskBand()
	path1 := `/home/ean/Desktop/vm/vm/temp/cutTile/test/12_3376_1700.tif`
	dsTile := fileDrv.Create(path1, tileSize, tileSize, 4, gdal.Byte, []string{})
	defer dsTile.Close()
	...
	dtile.IsFullTile = !hasEmptyCell
	//driverPNG, _ := gdal.GetDriverByName("PNG")
	//u1 := uuid.Must(uuid.NewV4())
	//p := fmt.Sprintf("/vsimem/%v.png", u1)
	//var ops []string
	//var data interface{}
	//pngds := driverPNG.CreateCopy(p, dsTile, 0, ops, gdal.DummyProgress, data)
	//defer pngds.Close()
	//var buf interface{} = make([]int8, 256*256)
	//err = pngds.RasterBand(1).IO(gdal.Read, 0, 0, 256, 256, buf, 256, 256, 0, 0)
	//if err != nil {
	...
	//driverPNG, _ := gdal.GetDriverByName("PNG")
	//u1 := uuid.Must(uuid.NewV4())
	//p1 := fmt.Sprintf("/vsimem/%v.png", u1)
	//var ops []string
	//var data interface{}
	//pngDT := driverPNG.CreateCopy(p1, dsTile, 0, ops, gdal.DummyProgress, data)
	newb := make([][]uint8, 0)
	for i := 1; i <= 4; i++ {
	...
	n := 0
	//p.Set(0, 0, color.RGBA{uint8(255), 0, 0, 255})
	//p.Set(255, 255, color.RGBA{uint8(255), 0, 0, 255})
	for y := 0; y < 256; y++ {
		for x := 0; x < 256; x++ {
	...
	pppp := `/home/ean/Desktop/vm/vm/temp/cutTile/test/y_x_255_255.png`
	pff, _ := os.Create(pppp)
	err = png.Encode(pff, p)
	if err != nil {
		log.Fatal("png encode error, err: ", err)
	}
	pff.Close()

	//pngDT := driverPNG.CreateCopy(p, dsTile, 0, ops, gdal.DummyProgress, data)
	//var pngbuf interface{} = make([]int8, 256*256)
	//pngDT.RasterBand(1).IO(gdal.Read, 0, 0, 256, 256, pngbuf, 256, 256, 0, 0)
	//fmt.Println(pngDT)
}
